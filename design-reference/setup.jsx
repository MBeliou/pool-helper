...
function OnbFooter({ P, next, back = true }) {
	return (
		<div style={{ flexShrink: 0, display: 'flex', gap: 12, padding: '12px 22px 34px' }}>
			...
			<div
				onClick={() => window.__nav && window.__nav('__onbnext')}
				style={{
					flex: 1,
					textAlign: 'center',
					padding: '15px',
	...
	return (
		<div
			style={{
				...
				background: on ? `${P.accent}15` : P.card,
...
const Body = ({ children }) => (
	<div style={{ flex: 1, overflow: 'hidden', padding: '20px 22px 0' }}>{children}</div>
);
...
function Welcome({ P }) {
	return (
		...
		>
			...
			<svg
				viewBox="0 0 402 402"
				style={{ position: 'absolute', top: -60, right: -120, width: 420, opacity: 0.13 }}
			>
				{[180, 140, 100, 60].map((r) => (
					<circle key={r} cx="201" cy="201" r={r} fill="none" stroke="#fff" strokeWidth="2" />
				))}
			</svg>
			<div
				style={{
			...
			>
				...
				<div style={{ fontSize: 17, opacity: 0.9, marginTop: 16, lineHeight: 1.4, maxWidth: 280 }}>
...
function ShapeGlyph({ kind, color }) {
	...
	if (kind === 'round')
		return <div style={{ ...base, width: 34, height: 34, borderRadius: '50%' }} />;
	if (kind === 'oval') return <div style={{ ...base, borderRadius: '50%' }} />;
	if (kind === 'kidney')
		return <div style={{ ...base, borderRadius: '45% 45% 55% 55% / 60% 60% 40% 40%' }} />;
	if (kind === 'free')
		return <div style={{ ...base, borderRadius: '62% 38% 50% 55% / 55% 48% 60% 50%' }} />;
	if (kind === 'L')
		return (
			<div
				style={{
					...
					clipPath: 'polygon(0 0,60% 0,60% 55%,100% 55%,100% 100%,0 100%)'
				}}
			/>
		);
...
function Shape({ P }) {
	...
	return (
		...
		>
			...
			<Body>
				<Title P={P} sub="Closest match — it drives the volume math">
					What shape is
					<br />
					your pool?
				</Title>
					...
					{shapes.map(([k, lb, on]) => (
						...
						>
							<div style={{ height: 40, display: 'grid', placeItems: 'center' }}>
								<ShapeGlyph kind={k} color={on ? P.accent : P.ink2} />
							</div>
							<span
								style={{ fontSize: 12.5, fontWeight: on ? 700 : 600, color: on ? P.accent : P.ink }}
							>
								{lb}
							</span>
						</div>
				...
				<div style={{ fontSize: 13, color: P.ink2, marginTop: 16, textAlign: 'center' }}>
					Freeform or odd shape? You'll just type the volume next.
				</div>
			</Body>
	...
	return (
		...
		>
			...
			<Body>
				...
				<div style={{ fontSize: 13, color: P.ink2, marginBottom: 7, fontWeight: 600 }}>
					Total volume
				</div>
				<div
					style={{
				...
				>
					<div style={{ color: P.accent, flexShrink: 0, marginTop: 1 }}>
						<Icon name="alert" size={18} sw={1.8} />
					</div>
					<div style={{ fontSize: 12.5, color: P.ink2, lineHeight: 1.35 }}>
						Don't know it? Switch to <b style={{ color: P.ink }}>Calculate it</b> and we'll work it
...
function Details({ P }) {
	...
	return (
		...
		>
			...
			<Body>
				<Title P={P} sub="These change the chemistry advice">
					A few
					<br />
					specifics
				</Title>
				<div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
					{groups.map(([g, opts]) => (
						<div key={g}>
							<div style={{ fontSize: 13, color: P.ink2, fontWeight: 700, marginBottom: 9 }}>
								{g}
							</div>
							<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
								{opts.map(([o, on]) => (
									<Pill key={o} P={P} on={on}>
										{o}
									</Pill>
								))}
							</div>
...
function Units({ P }) {
	const rows = [
		['Volume', 'Litres · gal · m³', 'Litres'],
		['Hardness / Alk', 'ppm · °fH · °dH', '°fH'],
		['Temperature', '°C · °F', '°C']
	];
	return (
		<div
		...
		>
			...
			<Body>
				<Title P={P} sub="So readings & doses match your kit">
					Units &<br />
					region
				</Title>
				<div style={{ fontSize: 13, color: P.ink2, fontWeight: 700, marginBottom: 9 }}>
				...
				<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 20 }}>
					<Pill P={P}>US</Pill>
					<Pill P={P}>UK / Imperial</Pill>
					<Pill P={P} on>
						Metric (most of world)
					</Pill>
				</div>
				<div
					style={{ background: P.card, borderRadius: 16, boxShadow: P.shadow, overflow: 'hidden' }}
				>
					{rows.map(([l, opts, v], i) => (
						<div
							key={l}
						...
						>
							<div>
								<div style={{ fontSize: 15, color: P.ink, fontWeight: 600 }}>{l}</div>
								<div style={{ fontSize: 11.5, color: P.ink2 }}>{opts}</div>
							</div>
							<span
								style={{
									display: 'flex',
	...
	return (
		...
		>
			...
			>
				...
				<div style={{ fontSize: 14, color: P.ink2, marginTop: 8, marginBottom: 24 }}>
