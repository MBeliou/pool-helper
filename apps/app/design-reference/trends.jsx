...
const SERIES = [
	{
		...
		note: 'creeping up',
		dir: 'up',
		s: [0.5, 0.5, 0.55, 0.58, 0.62, 0.66, 0.72, 0.74, 0.78, 0.82]
	},
	{
		key: 'fc',
		label: 'Free Cl',
		...
		note: 'dropping',
		dir: 'down',
		s: [0.7, 0.62, 0.55, 0.5, 0.45, 0.4, 0.36, 0.33, 0.31, 0.28]
	},
	{
		key: 'ta',
		label: 'Alkalinity',
		...
		note: 'steady',
		dir: 'flat',
		s: [0.5, 0.52, 0.48, 0.5, 0.51, 0.49, 0.5, 0.5, 0.49, 0.5]
	},
	{
		key: 'cya',
		label: 'CYA',
		...
		note: 'steady',
		dir: 'flat',
		s: [0.55, 0.55, 0.58, 0.57, 0.6, 0.59, 0.6, 0.6, 0.59, 0.6]
	}
];
			...
			<div style={{ flex: 1, overflow: 'hidden', padding: '0 18px' }}>
				<div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
					{SERIES.map((r) => (
						<div
							key={r.key}
							onClick={() => window.__nav && window.__nav('trenddetail')}
							style={{
								display: 'flex',
								alignItems: 'center',
								gap: 12,
						...
						>
							<div style={{ width: 86, flexShrink: 0 }}>
								<div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
									<span
										style={{ width: 8, height: 8, borderRadius: 999, background: tc(P, r.status) }}
									/>
							...
							<div style={{ flex: 1, minWidth: 0, height: 38 }}>
								<HChart P={P} series={r.s} color={tc(P, r.status)} h={38} gid={`spk-${r.key}`} />
							</div>
							<span
								style={{
									fontFamily: "'Space Grotesk',sans-serif",
			...
			<div style={{ flex: 1, overflow: 'hidden', padding: '14px 16px 0' }}>
					...
					>
						<span
							style={{
								...
								height: 8,
								borderRadius: 3,
								background: `${P.st.ok}33`,
								border: `1px dashed ${P.st.ok}`
							}}
						/>
						Ideal range 7.2–7.6
					</div>
				...
				>
					<div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
						...
						>
							<Icon name="spark" size={16} sw={1.9} />
						</div>
					...
					<div style={{ fontSize: 13, color: P.ink2, lineHeight: 1.4 }}>
						Fresh plaster leaches lime for ~12 months, nudging pH up about 0.1 a week. Expect
						frequent small acid doses early on — it settles.
					</div>
				</div>
			</div>
			<TabBar P={P} active="wave" />
